...
public class DList extends List {
   ...
   **/
    ...
    public void insertFront(Object item) {
        ...
        // new node prev points to head and next points to current head.next
        DListNode n = newNode(item, this, head.next.prev, head.next);
        // the previous head.next should point to the new node as well
        head.next.prev = n;
        head.next = n;
        // if empty list, then set head.prev to the new node. This doesn't change later.
        if(size==0) {
            head.prev = n;
        }
        size++;
